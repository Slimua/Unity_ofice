...
import type { ISlideDrawing } from '@univerjs/slides-drawing';
import { ISlideDrawingService } from '@univerjs/slides-drawing';
...
@OnLifecycle(LifecycleStages.Rendered, SlideDrawingUpdateController)
export class SlideDrawingUpdateController extends Disposable {
    ...
    private async _insertFloatImage(file: File) {
        ...
        const slideTransform = this._getImagePosition(width * scale, height * scale, sceneWidth, sceneHeight);

        if (slideTransform == null) {
            return;
        }

        const slideDrawingParam: ISlideDrawing = {
            unitId,
            subUnitId,
            drawingId: imageId,
            ...
            imageSourceType,
            source,
            transform: drawingPositionToTransform(slideTransform, this._selectionRenderService, this._skeletonManagerService),
            slideTransform,
        };

        this._commandService.executeCommand(InsertSheetDrawingCommand.id, {
            unitId,
            drawings: [slideDrawingParam],
    ...
    private _updateImageListener() {
        this._drawingManagerService.featurePluginUpdate$.subscribe((params) => {
            ...
            (params as IImageData[]).forEach((param) => {
                ...
                const slideDrawing = this._slideDrawingService.getDrawingByParam({ unitId, subUnitId, drawingId });

                // const imageDrawing = this._drawingManagerService.getDrawingByParam({ unitId, subUnitId, drawingId });

                if (slideDrawing == null) {
                    return;
                }

                const sheetTransform = transformToDrawingPosition({ ...slideDrawing.transform, ...transform }, this._selectionRenderService);
                ...
                const newDrawing: Partial<ISheetDrawing> = {
                    ...param,
                    transform: { ...slideDrawing.transform, ...transform, ...drawingPositionToTransform(sheetTransform, this._selectionRenderService, this._skeletonManagerService) },
        ...
        this.disposeWithMe(
            this._drawingManagerService.focus$.subscribe((params) => {
                if (params == null || params.length === 0) {
                    ...
                    this._slideDrawingService.focusDrawing([]);
                } else {
                    this._contextService.setContextValue(FOCUSING_COMMON_DRAWINGS, true);
                    this._slideDrawingService.focusDrawing(params);
                }
            })
        );
    }
    ...
    private _drawingAddListener() {
        this.disposeWithMe(
            this._slideDrawingService.add$.subscribe((params) => {
    ...
    private _registerDrawing(params: IDrawingSearch[]) {
        console.log('_registerDrawing');
        (params).forEach((param) => {
            const drawingParam = this._slideDrawingService.getDrawingByParam(param) as ISlideDrawing;

            if (drawingParam == null) {
                return;
            }

            const { slideTransform } = drawingParam;

            drawingParam.transform = drawingPositionToTransform(slideTransform, this._selectionRenderService, this._skeletonManagerService);
        });

        const unitId = params[0].unitId;

        this._drawingManagerService.registerDrawingData(unitId, this._slideDrawingService.getDrawingDataForUnit(unitId));
